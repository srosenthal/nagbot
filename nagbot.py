...
def main():
    ...
    instances = awsutil.list_ec2_instances()
...
def post_slack_summary_messages(instances, destination_slack_channel):
    ...
    message = "Hi, I'm Nagbot v1.01 :wink: My job is to make sure we don't forget about unwanted AWS servers and waste money!\n"
    ...
    send_message_to_slack(destination_slack_channel, message, slack_client)

    # TODO SJR skip this message if empty
...
def send_message_to_slack(destination_slack_channel, message, slack_client):
...
def is_past_date(str):
    str = str.replace(' (Entered by Nagbot)', '')
    if is_date(str):
